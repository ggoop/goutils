...
func TestAutoMigration(t *testing.T) {
	...
	now := time.Now()
	DB.Save(&EmailWithIdx{Email: "ggoop@example.org", UserAgent: "pc", RegisteredAt: &now})
	...
	if bigemail.Email != "ggoop@example.org" || bigemail.UserAgent != "pc" || bigemail.RegisteredAt.IsZero() {
